...
export function CourseSettingsStep({
  ...
  const [showDiscount, setShowDiscount] = useState(formData.discountPrice !== null);

  const calculateDiscount = () => {
    if (formData.discountPrice && formData.price > 0) {
      return Math.round(((formData.price - formData.discountPrice) / formData.price) * 100);
    }
    return 0;
  };
        ...
        <div className="p-4 rounded-lg border bg-muted/30 space-y-4">
          ...
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={() => {
                updateFormData({ isFree: true, price: 0, discountPrice: null });
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (showDiscount) {
                        updateFormData({ discountPrice: null });
                        setShowDiscount(false);
                      } else {
        ...
        <div className="flex items-center justify-between p-4 rounded-lg border bg-muted/30">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              formData.hasCertificate ? "bg-primary" : "bg-muted-foreground/30"
            }`}
          >
            <span
              className={`inline-block size-4 transform rounded-full bg-white transition-transform ${
                formData.hasCertificate ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </button>
        </div>
        ...
        <div className="p-4 rounded-lg border bg-muted/30 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                formData.maxStudents !== null ? "bg-primary" : "bg-muted-foreground/30"
              }`}
            >
              <span
                className={`inline-block size-4 transform rounded-full bg-white transition-transform ${
                  formData.maxStudents !== null ? "translate-x-6" : "translate-x-1"
                }`}
              />
            </button>
          </div>
...
function calculateCompletionScore(formData: CourseFormData): number {
  ...
  if (formData.isFree || formData.price > 0) score += weights.pricing;
